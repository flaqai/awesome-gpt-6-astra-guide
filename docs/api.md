# API integration — optional / API 进阶接入

[客户端优先入门](quickstart.md) · [English](../README.md) · [示例代码](../examples/README.md)

只有把模型接入自己的应用时才需要本页。使用 ChatGPT、Codex 客户端或通过 ChatGPT 登录 Codex CLI，不需要先配置这些 API 参数。
